...
const summaryLines = [
  ...
  "",
  "候選結果只寫入 data/award-discovery-candidates.json，不會顯示在網站上，也不會自動寫入 data/awards.json；status 為 review_needed 的項目仍需人工核對後才能手動加入正式資料。"
];
...
if (outcome.allFailed) {
  ...
  const candidates = mergeCandidates({
    ...
    requireAnyTitleTerms:config.requireAnyTitleTerms || [],
    normalize
  });
